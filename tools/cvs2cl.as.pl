...
sub derive_change_log ()
{
  ...
  while (my ($dir,$authorhash) = each %grand_poobah)
  {
    ...
    my $last_header_line = "nothing really";
    my $last_body = "nothing really";
    ...
    foreach my $time (@key_list)
    {
	  ...
	  # print ("1>$header_line<" . "\n1>$last_header_line<") ;
	  if( "$last_header_line" eq "$header_line" )
	  {
		$header_line = "";
	  }else
	  {
	  ...
	  $body =~ s/[ \t]+\n/\n/g;
	  if( $last_body eq $body && $header_line eq "" )
	  {
	      $body = "" ;
	  }else
	  {
...
sub parse_date_author_and_state ()
{
  ...
  my ($year, $mon, $mday, $hours, $min, $secs, $timezone, $author, $state, $rest) =
    $line =~
